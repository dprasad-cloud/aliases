...
# 2. Process and Force 45-char Trim
($1$2) in u_cpu {
   uc=to_m(u_cpu[$1$2]); lc=to_m($3);
   um=to_mi(u_mem[$1$2]); rm=to_mi($4); lm=to_mi($5);

   p_name = $2;
   gsub(/[[:space:]]/, "", p_name); # Clean hidden spaces

   if (length(p_name) > 45) {
       display_pod = substr(p_name, 1, 42)"..."
   } else {
       display_pod = p_name
   }
   ...
   # %-45.45s handles the strict display limit
   printf "%-10s | %-45.45s | C: %-5s/%-5s (%3d%%) | M: %-7s | R: %-5s (%3d%%) | L: %-5s (%3d%%) | %s\n",
