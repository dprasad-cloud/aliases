...
# --- DATA COLLECTION ---
for i in 1 2; do
    # Build SSH host and select namespace/temp file based on iteration
    if [ $i -eq 1 ]; then
    ...
    END { for (k in cpu_req) print k"\t"cpu_req[k]"\t"cpu_lim[k]"\t"mem_req[k]"\t"mem_lim[k]"\t"count[k] }' | sort > "$OUTPUT_FILE"
done
...
awk '
BEGIN {
    s1 = ENVIRON["TB1"]; s2 = ENVIRON["TB2"];
    printf "%-35s | %-26s | %-26s | %-26s | %-26s\n", "GROUP", "CPU REQUEST (m)", "CPU LIMIT (m)", "MEM REQUEST (Mi)", "MEM LIMIT (Mi)";
    printf "%-35s | %-12s %-12s | %-12s %-12s | %-12s %-12s | %-12s %-12s\n",
...
}' "$TMP1" "$TMP2" | { IFS= read -r h1; IFS= read -r h2; printf "%s\n%s\n" "$h1" "$h2"; sort -k1,1; }
