...
if [[ "$FILTER" == "all" || "$FILTER" == "ALL" || -z "$FILTER" ]]; then
    ...
    # Support regex-style filtering if spaces are used
...
function to_m(val) {
   ...
   # If small value (e.g., 2 or 0.5), it is Cores -> convert to millicores
   # If large value (e.g., 2500), it is already millicores
   if (v + 0 < 50) {
       return v * 1000
   }
   return v + 0
...
($1$2) in u_cpu {
   ...
   printf "%03d|%-12s|%-27.27s|C: %-6s %-12s %-15s|M: %-7s %-12s %-15s|%s\n",
