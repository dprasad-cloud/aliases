...
kubectl get pod -A --sort-by='.metadata.creationTimestamp' | tac | grep -iE "$SEARCH_TERM_CLEANED" | column -t

# Execute the function with the script's arguments
echo -e "\n \ncommand(s):\n\t kubectl get pod -A | tac | grep -iE \"$SEARCH_TERM_CLEANED\" | column -t"
echo -e "\n"
